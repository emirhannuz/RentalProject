...
namespace Core.Utilities.Helpers.FileHelpers.Images
{
    public class ImageFileHelper : IImageFileHelper
    {
        ...
        {
            //var result = ChecIfAvailableFileExtension(formFile);
            var result = HelperRules<string>.Run(CheckIfAvailableFileExtension(formFile));
            if (result != null)
            {
                return result;
            }

            var path = WriteFileToFolder(formFile);
            return new SuccessDataResult<string>(data: path.Result);

        }
        public async Task<string> WriteFileToFolder(IFormFile formFile)
        {
